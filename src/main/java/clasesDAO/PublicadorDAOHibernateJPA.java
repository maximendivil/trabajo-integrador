...
public class PublicadorDAOHibernateJPA extends GenericDAOHibernateJPA<Publicador> implements PublicadorDAO {
	...
	public List<Cartelera> obtenerCarteleras(long id) {
		Query q = EMF.getEMF().createEntityManager().createQuery("Select * from Publicador pr INNER JOIN CartelerasHabilitadas ch on pr.id=ch.PUBLICADOR_ID INNER JOIN Cartelera c on ch.CARTELERA_ID=c.id");
